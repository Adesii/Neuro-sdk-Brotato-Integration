...
func validate(command: String, message_data, state: Dictionary): # -> ExecutionResult
	...
	if result == null:
		...
		return load("res://mods-unpacked/Adesi-NeuroIntegration/neuro-sdk/websocket/execution_result.gd").mod_failure("Action failed. An error occurred.")
...
func _validate(_command: String, _data, _state: Dictionary): # -> ExecutionResult
	...
	return load("res://mods-unpacked/Adesi-NeuroIntegration/neuro-sdk/websocket/execution_result.gd").mod_failure("IncomingMessage.validate() is not implemented.")
