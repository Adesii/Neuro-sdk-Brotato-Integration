extends "res://mods-unpacked/Adesi-NeuroIntegration/neuro-sdk/messages/api/incoming_message.gd"
class_name Action
...
func _validate(_command: String, message_data, state: Dictionary): # -> ExecutionResult
	if message_data == null:
		return load("res://mods-unpacked/Adesi-NeuroIntegration/neuro-sdk/websocket/execution_result.gd").vedal_failure("Action failed. Missing command data.")

	var action_id = message_data.get_string("id");
	if not action_id:
		return load("res://mods-unpacked/Adesi-NeuroIntegration/neuro-sdk/websocket/execution_result.gd").vedal_failure("Action failed. Missing command field 'id'.")
	...
	if action_name == null or action_name == "":
		return load("res://mods-unpacked/Adesi-NeuroIntegration/neuro-sdk/websocket/execution_result.gd").vedal_failure("Action failed. Missing command field 'name'.")
...
func _report_result(state: Dictionary, result) -> void:
	...
	get_tree().root.get_node(WEBSOCKET_NODE_PATH).send(load("res://mods-unpacked/Adesi-NeuroIntegration/neuro-sdk/messages/outgoing/action_result.gd").new(id, result))
